...
define('CITATION_MANAGER_VERSION', '0.9.2');
...
function citationhtml($citation) {
	...
	if ($cit_url) {
		...
		if (get_option('cit_targetblank') == 1)
			$html .= 'target="_blank" ';
		$html .= 'href="' . $cit_url . '" title="' . (($cit_title) ? htmlentities($cit_title, ENT_QUOTES) : htmlentities($cit_publication, ENT_QUOTES)) . '">';
	}
...
function citation_dump() {
	...
	foreach ($citations as $citation) {
		if ($citation['ID'] != $curpost) {
			...
			$html .= '<a href="' . get_permalink($citation['ID']) . '" title="' . $citation['post_title'] . '">' . $citation['post_title'] . '</a>' . "\n";
